...
func start() -> Script:
	...
	player.level.set_deferred("process_mode", Node.PROCESS_MODE_DISABLED)
	player.collision_mask = 0
	player.collision_layer = 0
	player.velocity = Vector2.ZERO
	if player.held_item != null:
		player.held_item.reparent(player.get_parent())
	...
	MusicPlayer.stop()
	UISoundPlayer.stream = preload("uid://cpvqy2k0d2cju")
	UISoundPlayer.play()
	...
	_death_timer.timeout.connect(_fall.bind(grav_comp),
			ConnectFlags.CONNECT_ONE_SHOT)
	
	return


func _fall(grav_comp: GravityComponent) -> void:
